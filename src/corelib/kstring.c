...
{
    size_t beg = 0, length = ks_size(*line), end = length;
    while (beg < length && isspace(ks_A(*line, beg))) ++beg;
    if (beg == length) {
        ks_set_size(line, 0);
        return;
    }
    while (end && isspace(ks_A(*line, end - 1))) --end;
    assert(beg < end && end <= length);
    if (beg || (length - end)) {
        size_t pos = 0;
        for (size_t i = beg; i < end; ++i, ++pos) ks_A(*line, pos) = ks_A(*line, i);
        ks_set_size(line, pos);
    }
}
