...
split_read_worker(void* arg)
{
	...
	new_kvec(vec_adjovlp, adjovlp);
	new_kvec(vec_bad_region, blist);
	while (1) {
		m4v = get_next_range(data->m4v, 
							 ...
							 &nm4);
		if (!m4v) break;
        if (data->clear_ranges[m4v[0].sid-1].size == 0) continue;
		preprocess_m4v(m4v, &nm4);
		kv_clear(adjovlp);
		add_and_filter_overlaps(m4v, nm4, data->clear_ranges, &adjovlp);
		kv_clear(blist);
		detect_subread(m4v[0].sid - 1, kv_data(adjovlp), kv_size(adjovlp), &blist);
		left = data->clear_ranges[m4v[0].sid - 1].left;
		right = data->clear_ranges[m4v[0].sid - 1].right;
		...
		const int num_threads)
{
	...
	SplitReadData* sr_data = new_SplitReadData(kv_data(m4v),
							 kv_size(m4v),
							 kv_data(idx_range),
							 kv_size(idx_range) - 1,
							 clipped_ranges,
